...
<div class="container mt-5">
    <h2>Ajouter un aliment au régime: <?= esc($regime['nom']) ?></h2>
    ...
    <form action="<?= site_url('admin/regime-aliment/store/' . $regime['id']) ?>" method="POST" class="form mt-4">
        <?= csrf_field() ?>

        <div class="mb-3">
            <label for="aliment_id" class="form-label">Aliment <span class="text-danger">*</span></label>
            <select id="aliment_id" name="aliment_id" class="form-select" required>
                <option value="">-- Sélectionnez un aliment --</option>
                <?php foreach ($aliments as $aliment): ?>
                    <option value="<?= $aliment['id'] ?>" <?= old('aliment_id') == $aliment['id'] ? 'selected' : '' ?>>
                        <?= esc($aliment['nom']) ?> (<?= ucfirst($aliment['type_aliment']) ?>)
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                ...
                <div class="input-group">
                    <input type="number" id="pourcentage_viande" name="pourcentage_viande" class="form-control" step="0.01" min="0" max="100" required value="<?= old('pourcentage_viande', 0) ?>">
            ...
            <div class="col-md-4 mb-3">
                ...
                <div class="input-group">
                    <input type="number" id="pourcentage_poisson" name="pourcentage_poisson" class="form-control" step="0.01" min="0" max="100" required value="<?= old('pourcentage_poisson', 0) ?>">
                    <span class="input-group-text">%</span>
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                ...
                <div class="input-group">
                    <input type="number" id="pourcentage_volaille" name="pourcentage_volaille" class="form-control" step="0.01" min="0" max="100" required value="<?= old('pourcentage_volaille', 0) ?>">
                    <span class="input-group-text">%</span>
                </div>
            </div>
        </div>
